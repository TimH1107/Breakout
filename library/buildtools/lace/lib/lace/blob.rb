		...
		def self.fetch( config, server_id, blob_path, required_hash )
			...
			if !File.exists?( blob_content_path )
				...
				# check if the local path exists and if the hash file is present:
					...
					if blob_server.user
						svn_command_line = "svn --username #{blob_server.user} --password #{blob_server.password} --non-interactive --no-auth-cache export #{svn_remote_path} #{local_path}"
					else
						svn_command_line = "svn export --non-interactive #{svn_remote_path} #{local_path}"
					end
